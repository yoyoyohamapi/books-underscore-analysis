...
显然，二者就构成了上面我们提到的继承（层次递进）关系，在js中，原型和对象间的联系链条是通过对象的`—__proto__`属性来完成的。举个更具体的例子，学生对象（student）的原型是人（person），因为学生源于人，在js中我们可以这样实现二者的继承关系：
