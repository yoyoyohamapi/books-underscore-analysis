...
### `_.now()`：方法返回自1970年1月1日 00:00:00 UTC到当前时间的毫秒数
...
该方法会优先尝试使用ES5.1提供的`Date.now()`方法。
...
### `_.result(obj, prop, fallback)`：如果`prop`是`obj`的成员方法，则调用之，否则获得`prop`属性。同时，若`obj.prop`为`undefined`，可以传递一个`fallback`做为后置处理。
...
### `_.uniqueId(prefix)`： 根据传入的`prefix`，生成唯一id
...
> 值得注意的是，从`createEscaper`源码中我们也看到了，`String.prototype.replace(regexp|substr, newSubStr|function)`的第二个参数不仅能够接受字符串对匹配的到字符子串进行**直接替换**，还能接受函数进行**间接替换**，该函数接受的一个参数，其代表的是匹配到的字符子串。
