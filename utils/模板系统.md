...
* 支持传递一个变量标记，这样，模板编译的时候，将不会用到`with`来限定作用域，从而显著提升模板性能：
  ...
  _.template("Using 'with': <%= data.answer %>", {variable: 'data'})({answer: 'no'});
    // => "Using 'with': no"
  ```
* 在返回的模板函数中，提供了一个`source`属性，来支持JST（JvaScript Template）。JST是嵌入了js代码的模板文件，该文件保存在服务端，客户端（浏览器）可以直接请求该文件并放入`<script></script>`标签中执行，待执行的内容需要通过`<%%>`包裹：
...
- [Javascript Templates (JST)](https://embedthis.com/goahead/doc/users/jst.html)
