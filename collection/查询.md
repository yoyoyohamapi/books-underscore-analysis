...
除了 `createIndexFinder` 以外，underscore 还内置了一个 `createPredicateIndexFinder` 的工厂函数，该函数甚至可以看做是 `createIndexFinder` 的增强版，因为其不仅能查询直接量在集合中的位置，也支持通过一个真值检测函数查找位置。`createPredicateIndexFinder` 接受一个参数，是：
...
`_.findKey`： 利用真值检测函数，获得对象中第一个满足条件的 key
--------------------------------------------------------------
...
`_.where`：类似于 SQL 中的 where 限定符，根据指定条件，返回集合中所有满足条件的元素。
-------------------------------------------------------------------------------------
...
下面我们看到 underscore 中 `_.where` 中的实现：
...
`_.findWhere`：与 `where` 类似，但只返回第一条查询到的记录
----------------------------------------------------------
