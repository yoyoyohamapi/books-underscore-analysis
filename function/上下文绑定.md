...
2. 创建待绑定的函数
  ...
  fBound();
  ```
  当fbound调用时，会对执行上下文进行校正：
  ```js
    this instanceof fNOP? this: oThis || this
  ```
  如果：
  - 当前的上下文`this`是`fNOP`的一个实例，则不需要改动执行上下文，这其实说的是，如果`this`指向了一个由`new`运算符构造的对象，那么不再需要变更上下文：
    ```js
      var obj = {
        foo: foo.bind(null)
      }
...
- 如果执行上下文是绑定函数的实例，比如：
  ...
  var person = new Person("wxj");
  var 
    return executeBound(showName, Person, person, this);
  };
  person.bound();
  ```
