...
当函数`foo`调用时，亦即`foo()`语句执行时，其所在的__上下文__是全局对象，所以，`this`绑定到了全局对象（在浏览器环境的话就是`window`对象，`foo()`也相当于是`window.foo()`）。再看下面的一个代码：
```js
function foo() {
  console.log(this.a);
}
...
并且，类似于MDN中提供的polyfill，`executeBound`也考虑到了当绑定后的函数`boundFunc`作为构造函数被`new`运算的情形，进行了执行上下文的修正。另外，为了支持链式调用，所以有如下语句：
```js
var result = sourceFunc.apply(self, args);
if (_.isObject(result))
    return result;
...
#### `_.bindAll`：绑定对象`obj`的所有指定成员方法中的`this`到`obj`
先看到下面这样一个例子：
```js
// 我们模拟一个DOM元素
var button = {
...
```
悲剧，当我们把成员方法作为`setTimeout`的回调传入后，`this`在运行时的绑定事与愿违。记住，`button.onClick`不是`button.onClick()`，回调函数执行时的上下文不再是`button`对象，为此，我们就需要绑定`button`所有成员方法的`this`到该对象上，underscore通过`_.bindAll`来实现这个目标。
